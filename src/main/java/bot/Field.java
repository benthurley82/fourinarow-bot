...
public class Field
{
	...
	/**
	 * Constructor to clone a new field from an existing one
	 * 
	 * @param field
	 */
	public Field(Field field)
	{
		this(field.getNrColumns(), field.getNrRows());
		parseFromString(field.toString());
	}
	 ...
	 * 
	 * @param column
	 *            the column for the move you want to undo
	 * @return
	 */
	public boolean undoAddDisc(int column)
	{
	 ...
	 * 
	 * @return : Returns true when field is empty, otherwise returns false.
	 */
	public boolean isEmpty()
	{
		for (int x = 0; x < mCols; x++)
			for (int y = 0; y < mRows; y++)
				if (mBoard[x][y] != 0)
					return false; // At least one cell is filled
		// All cells are filled
		return true;
	}
	 ...
	 * 
	 * @return : Returns true when given column is full, otherwise returns
	 *         false.
	 */
